...
def parse_message_string(pkg_name, msg_name, message_string):
    ...
    gijsvandenhoven_hack_measuring_fields_into_message(fields, pkg_name)
...
def gijsvandenhoven_hack_measuring_fields_into_message(fields, pkg_name):
    # ' fields '  is a list of Field objects.
    # We want to add two fields to _every_ message that gets built.
    # Which we do here, at the parsing of a .msg file, so that all the other complex processes generate code for it as needed.
    # It may be necessary later to allow opting in or out of this in the .msg file.
    # The reason why as of now, the default is opt-in, is because the purpose of these fields is analysing the performance of arbitrary existing ROS systems.
    # And it is not possible to know of an arbitrary system easily which messages it depends on, as far as I am aware.
    # You could find out from the package.xml files which custom messages do, but not the built-in ROS messages (?)

    # The regex is strict, not allowing double-underscore, or leading/trailing underscores.
    # To still stand a decent chance of not introducing duplicate field names, prefix it with my uncommon last name.
    # An actual error check against duplicates is performed in the MessageSpecification Constructor.

# todo: consider nested vandenhoven struct
# This is tricky because "structs"  are other messages, meaning to do this
# every package needs a dependency on some message package defining this nested struct.
    tsField = Field(Type('int64', context_package_name=pkg_name), 'vandenhoven_timestamp', None)
    idField = Field(Type('int64', context_package_name=pkg_name), 'vandenhoven_identifier', None)
    publisher_hash_field = Field(Type('int32', context_package_name=pkg_name), 'vandenhoven_publisher_hash', None)

    fields.append(tsField)
    fields.append(idField)
    fields.append(publisher_hash_field)
