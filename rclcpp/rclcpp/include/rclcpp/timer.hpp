...
public:
  ...
  explicit TimerBase(
    ...
    const TimerOptions& options = TimerOptions());
...
protected:
  ...
  rclcpp::IJitterTracker::UniquePtr jitter_tracker_;

private:
  void timer_activate_callback(int64_t time_since_last_activate, int64_t intended_activation_time);

  friend void rcl_timer_callback(rcl_timer_t *, int64_t, int64_t, void *);
};
...
public:
  ...
  : TimerBase(clock, period, context, TimerOptions()), callback_(std::forward<FunctorT>(callback))
  {
  }

  explicit GenericTimer(
    Clock::SharedPtr clock, std::chrono::nanoseconds period, FunctorT && callback,
    rclcpp::Context::SharedPtr context, const TimerOptions& options
  )
  : TimerBase(clock, period, context, options), callback_(std::forward<FunctorT>(callback))
  {
  }
...
public:
  ...
  : GenericTimer<FunctorT>(
      std::make_shared<Clock>(RCL_STEADY_TIME), period, std::move(callback), context, TimerOptions())
  {}
  ...
  : GenericTimer<FunctorT>(
    std::make_shared<Clock>(RCL_STEADY_TIME), period, std::move(callback), context, options)
  {}
