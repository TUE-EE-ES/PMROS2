...
Node::create_wall_timer(
  ...
  rclcpp::callback_group::CallbackGroup::SharedPtr group) 
{
  const rcl_node_t * interface = rclcpp::node_interfaces::get_node_base_interface(*this)->get_rcl_node_handle();
  TimerOptions t(interface);
  return create_wall_timer(period, std::move(callback), t, group);
}
...
Node::create_wall_timer(
  std::chrono::duration<DurationRepT, DurationT> period,
  CallbackT callback,
  const rclcpp::TimerOptions& timer_options,
  rclcpp::callback_group::CallbackGroup::SharedPtr group)
{
