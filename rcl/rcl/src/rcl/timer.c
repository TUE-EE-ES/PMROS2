...
typedef struct rcl_timer_impl_t
{
  ...
  // used with the user supplied callback for callback-ing member functions. This is more accurately seen as a void *, but atomically handling those.. I don't know how to (fix me, experienced RCL programmer!)
  atomic_int_least64_t bound_instance;
  ...
  rcl_allocator_t allocator)
{
  ...
  atomic_init(&impl.bound_instance, 0);
...
rcl_timer_bind_instance(rcl_timer_t * timer, void * instance) {
  rcutils_atomic_store(&timer->impl->bound_instance, (atomic_int_least64_t)instance);
}
...
rcl_timer_call(rcl_timer_t * timer)
{
  ...
  rcl_time_point_value_t intended_activation_ns = rcutils_atomic_exchange_int64_t(&timer->impl->next_call_time, next_call_time);

  if (typed_callback != NULL) {
    int64_t since_last_call = now - previous_ns;
    void * bound_instance = (void*)rcutils_atomic_load_int64_t(&timer->impl->bound_instance);
    typed_callback(timer, since_last_call, intended_activation_ns, bound_instance);
  }
  return RCL_RET_OK;
}
